...
class HomeData:
    ...
    def _read_df(self, split: Literal["train", "test"] = "train"):
        if split == "train":
            ...
            target = df[self.target_col] / 10000
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument(
        "--target-col", default="SalePrice", type=str, help="target column"
    )
    parser.add_argument(
        "--drop-cols",
        default=["LotFrontage", "MasVnrArea", "GarageYrBlt"],
        type=list,
        help="drop columns",
    )
    parser.add_argument(
        ...
        default="min",
        type=str,
        help="numeric column filling strategy (mean, min, max)",
    )
